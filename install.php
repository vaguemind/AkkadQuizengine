<?php if(isset($_GET['alldelete'])) { unlink("install.php"); unlink("install1.php"); unlink("install2.php"); unlink("css/home.css"); header("location: admin/index.php"); } ?>
...
<?php
	if(isset($_POST['submitandcreateafileformepleaseiamreadytoinstallyouradmintheme']))
	{
		...
		$stringData = 
		" <?php /*
		// Author:- Ahmed Elakkad
		// Version :- 1.0 
		// Project Name :- Quiz Engine script under GPL license 
		// You have the right to modify,use and re-distribute this script
		// without informing anyone.
		// As a curtsey's you can keep my name there as it is.
		*/
		...
		// --- Facebook Share Link --- //
		define('FACEBOOK_LINK','http://www.ahmedelakkad.website');
		// --- Facebook Share Query --- //
		define('FACEBOOK_QUERY','Jquery Quiz Engine Results (www.ahmedelakkad.website)');
		?>";
		...
		Unable to make database connection, delete config.php and try again with right credentials</p>';		
			...
			if(isset($conn))
			{
				$sql = mysql_query("CREATE TABLE questions(
				...
				option4 VARCHAR(300),
				answer VARCHAR(300),
				connection_meta smallint(3),
				created_on datetime NOT NULL,
  				created_by varchar(100) NOT NULL,
  				status varchar(50) NOT NULL,
				option_value VARCHAR(500))");
				
				$sql = mysql_query("CREATE TABLE tut_users(
				...
				$sql = mysql_query("CREATE TABLE quizes(
				id INT NOT NULL AUTO_INCREMENT, 
				PRIMARY KEY(id),
				...
				final_status varchar(30) NOT NULL DEFAULT 'false')");
				
				
		if($sql)
		{
			$success =  '<h2>You got it !! </h2>Necessary tables created <a href="#" id="create">Next</a>';		
		}
		else
		{
		$error1 = '<b>Unable to create tables Make sure database name you entered does exists <br /> Delete the config.php file and follow the process again <input type="hidden" name="finish" value="finish" id="finish"/><input type="submit" name="click" value="Delete Now" id="click" onclick="oops();" /></b>';
		}
		
		}
		else
		{
		$error = "Unable to create file please do it manually by copying the entire code or <b>contact me</b> ";
		}
		}
			else
			{
...
<html lang="name">
<head>
<title>Installing AkkadQuizEngineAdmin</title>

	<meta http-equiv="Content-type" content="text/html; charset=UTF-8" />	
	<meta name="author" content="installing AkkadQuizEngineAdmin" />
	<meta name="robots" content="installing AkkadQuizEngineAdmin">
	<meta name="description" content="installation page for AkkadQuizEngineAdmin"/>
...
<div id="headerwrap">
	<div id="header">
		<h1><a href="../index.php">AkkadQuizEngineAdmin</a></h1>
	</div>
	...
	<div id="install-main-content">
		...
		<div id="final-msg-ajax"></div>
			...
			<div id="rest">
				...
				to create awesome quiz by using Akkad Quiz Engine</p>
...
<div id="headerwrap">
	...
	<div id="install-main-content">
		...
		<h3>Don't Worry just follow normal steps ! you will be up and running soon</h3>	
			<p>File config.php doesn't exists you can hit go to create a config.php file dynamically
